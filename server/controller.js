...
// This is the same syntax as const CONECTION_STRING = process.env.CONNECTION_STRING
const {CONNECTION_STRING} = process.env; // Destructuring CONNECTION_STRING from our .env file
...
module.exports = {
    getUserInfo: (req, res) => {
        ...
        WHERE u.user_id = ${userId};`)
            .then((dbRes) => {
                res.status(200).send(dbRes[0]); // Postgres will always put your data at the [0] index of your dbRes
            })
            .catch((err) => {
                console.log(err);
            })
    ...
    requestAppointment: (req, res) => {
        ...
        VALUES(${clientId}, '${date}', '${service}', 'Dang, tabnine is sick', false, false)
        RETURNING *;`)
        .then((dbRes) => {
            res.status(200).send(dbRes[0]);
        })
